...
1)  **由 $v_\pi \to q_\pi$ 更稳健**：先评估 $v_\pi$（线性系统或 DP
    迭代），再按"一步展开式"计算 $q_\pi$，数值上更稳且便于并行。\
2)  **直接 $q$-迭代**：适合与控制/改进策略无缝衔接（如 Q-learning 的
    off-policy 更新）。\
3)  **近似表示**：在大状态-动作空间中，$q_\pi$ 更适合用函数逼近（如
    NN）直接拟合。\
4)  **探索-利用权衡**：基于 $q$ 的 $\varepsilon$-greedy/softmax
    更直观（直接对动作打分）。
